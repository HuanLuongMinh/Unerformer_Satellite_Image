...
resnext101_32x16d.fb_swsl_ig1b_ft_in1k, loss = CombinedLoss (CrossEntropy +
Dice), BASE_LR = 1e-5.

File này độc lập với src/train_unet_former_reshnet101_32.py (bản CE thuần),
src/train_cnn_resnext101_32.py và src/train_cnn.py — chỉ khác loss function
(CombinedLoss thay vì CE thuần) và model import (model riêng
unet_former_reshnet101_32_combineLoss.py, kiến trúc giống 100% bản gốc), toàn
bộ pipeline DDP/AMP/checkpoint/resume/early-stopping/visualization giữ nguyên
để các thí nghiệm cũ không bị ảnh hưởng khi chạy lại.
...
Usage (Kaggle, 2x T4):
    ...
    torchrun --nproc_per_node=2 src/train_unet_former_reshnet101_32_combineLoss.py --config configs/unet_former_reshnet101_32_combineLoss/luot1_500.yaml --dry-run
    torchrun --nproc_per_node=2 src/train_unet_former_reshnet101_32_combineLoss.py --config configs/unet_former_reshnet101_32_combineLoss/luot1_500.yaml --resume <work_dir>/latest_checkpoint.pth
"""
...
from src.models.unet_former_reshnet101_32_combineLoss import build_model
...
def main():
    ...
    # CombinedLoss = CrossEntropy + Dice, theo yêu cầu thí nghiệm CombinedLoss.
    ...
    log(f"Training {tr['MAX_ITERS']} iterations | "
        f"encoder={mdl['ENCODER']} | loss=CombinedLoss(CE+Dice) | split={ds['TRAIN_SPLIT_FILE']}")
