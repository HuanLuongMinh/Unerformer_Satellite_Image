...
Kiến trúc giống 100% src/models/unet_former_reshnet101_32.py — file này tách
riêng chỉ để dùng cho thí nghiệm CombinedLoss (CE + Dice), không sửa/ảnh hưởng
file gốc (vẫn train với CE thuần). Decoder được port từ
D:\\Claude\\implement\\UnetFormer_v2\\unetformer.py, KHÔNG mang theo bất kỳ phần
DAPCN nào (không boundary loss, không prototype/contrastive learning, không
dynamic anchor point) — chỉ gồm GlobalLocalAttention + Block (MLP) + Weighted
Fusion (WF) + FeatureRefinementHead, đúng kiến trúc UNetFormer gốc.
...
nên src/train_unet_former_reshnet101_32_combineLoss.py dùng được pattern train
giống các script train_*.py hiện có.
"""
